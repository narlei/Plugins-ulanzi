...
function Handle-Request {
  ...
  if ($Method -eq "POST" -and $path -eq "/play") {
    $index = [int](Get-JsonField -Obj $body -Name "index")
    $res = $soundpad.PlaySound($index).GetAwaiter().GetResult()
  ...
  if ($Method -eq "POST" -and $path -eq "/play-random") {
    $categoryIndex = Get-JsonField -Obj $body -Name "categoryIndex"
  ...
  if ($Method -eq "POST" -and $path -eq "/record-start") {
    $res = $soundpad.StartRecordingMicrophone().GetAwaiter().GetResult()
    if ($res.IsSuccessful) { $recording = $true }
    return @{ Code = $(if ($res.IsSuccessful) { 200 } else { 500 }); Data = @{ ok = $res.IsSuccessful; recording = $recording; error = $res.ErrorMessage } }
  }

  if ($Method -eq "POST" -and $path -eq "/record-stop") {
    $res = $soundpad.StopRecording().GetAwaiter().GetResult()
    if ($res.IsSuccessful) { $recording = $false }
    return @{ Code = $(if ($res.IsSuccessful) { 200 } else { 500 }); Data = @{ ok = $res.IsSuccessful; recording = $recording; error = $res.ErrorMessage } }
  }

  if ($Method -eq "POST" -and $path -eq "/load-soundlist") {
    $filePath = "$(Get-JsonField -Obj $body -Name "path")"
